...
class C1
{
    public:
        C1()
            noexcept(false)
            : a1(new A), a2(new A) // UndCC_Violation - if a2 memory allocation
              // fails, a1 will never be deallocated
            {
            }
        C1(A* pA1, A* pA2)
            noexcept : a1(pA1), a2(pA2) // UndCC_Valid - memory allocated outside of C1
...
class C2
{
        ...
        C2() noexcept(false) : a1(nullptr), a2(nullptr)
    {
        ...
        {
            throw; // UndCC_Violation - whenever a2 allocation throws an
...
class C3
{
        ...
        C3() noexcept(false) : a1(nullptr), a2(nullptr), file("./filename.txt")
    {
        ...
        catch (std::exception& e)
        {
            ...
            a2 = nullptr;
            file.close();
            throw; // UndCC_Valid - all resources are deallocated before the
        ...
        C4() : x(0), y(0)
    {
            ...
            noexcept(false) : x(first), y(second)
        {
            CheckPreconditions(x,
                    y); // UndCC_Valid - if constructor failed to create a
