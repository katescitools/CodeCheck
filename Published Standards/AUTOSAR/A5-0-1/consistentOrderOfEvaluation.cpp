...
{
    std::uint16_t x = arr[idx] + idx++; // UndCC_Violation
}
...
{
    std::uint8_t i = 0;
    std::uint8_t x = Func(i++, i); // UndCC_Violation
}
...
{
    p->TaskStartFn(p++); // UndCC_Violation(Win) UndCC_FalseNeg(Lin,Mac)
}
...
{
    std::uint16_t x = G(a) + Z(a);  // UndCC_FalseNeg
}
// could be written as
...
{
    s.push(Pop(s) - Pop(s));  // UndCC_FalseNeg
}
...
{
    std::int32_t y = 4;
    x = y = y++; // UndCC_Violation(Win) UndCC_FalseNeg(Lin,Mac) It is undefined whether the final value of y is 4 or 5 
}
...
{
    std::uint32_t z = MAX(i++, j);  // UndCC_FalseNeg
}
...
extern std::uint16_t foo(std::uint8_t, std::uint8_t) noexcept(false);
void F11(std::uint8_t a) noexcept(false)
{
    std::uint16_t x = foo(a, a); // UndCC_Valid
}

extern std::uint16_t foo2(std::uint8_t, std::uint8_t) noexcept(false);
void F12(std::uint8_t a) noexcept(false)
{
    std::uint16_t x = foo2(a++, a);  // UndCC_Violation
}
