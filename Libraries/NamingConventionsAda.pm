package CodeCheck::Libraries::NamingConventionsAda;
use strict;
use Exporter;
use experimental 'smartmatch';

our @ISA	= qw(Exporter);
our @EXPORT = qw(setOptions registerNamingErrors getNamingDescription setNamingOptions checkNamingRules getPrimaryKind adaNamingCheck NAMEERROR);
...
#get refs that are valid
sub getValidRefs{
  my($ent) =@_;
  my @list = ();
  my @dupList = ();
  foreach my $ref ($ent->refs()){
    my $textL = $ref->file->lexer->lexeme($ref->line,$ref->column);
    if(defined $textL){
      my $text = $textL->text;
      my $loc = $ref->file . $ref->line . $ref->column;
      if(lc $ent->name eq lc $text && !($loc ~~ @dupList)){    
        push @list, $ref;
        push @dupList, $loc
      }
    }
...
sub adaNamingCheck{
  ...
  my $deepSearch = isAdaDeepSearch($check,$varkindNameArray);

  if($deepSearch)
  {
    #first do the original check on the def
    my $trueName = $varRef->file->lexer->lexeme($varRef->line,$varRef->column)->text;
    my $originalErrors = checkNamingRules($trueName,$varkindNameArray,$check);

    $check->violation($var,$varRef->file,$varRef->line,$varRef->column,NAMEERROR,$CodeCheck::Libraries::NamingConventions::kindNameLookup{$varkindNameArray},$trueName,$originalErrors) if $originalErrors;

    my @oErrors = split('\.', $originalErrors);

    

    #update to make it check every instance with correct casing
    my @refList = getValidRefs($var);
    foreach my $useRef (@refList){
      $trueName = $useRef->file->lexer->lexeme($useRef->line,$useRef->column)->text;
      my $errors = checkNamingRules($trueName,$varkindNameArray,$check);
      my @errorList = split('\.', $errors);
      #$check->violation($var,$varRef->file,$varRef->line,$varRef->column, $errors . scalar @errorList);
      foreach my $e (@errorList)
      {
        
        #if we haven't seen this error in the original, then mark it
        if( !($e ~~ @oErrors) )
        {
          $check->violation($var,$useRef->file,$useRef->line,$useRef->column, "Use of " . NAMEERROR,$CodeCheck::Libraries::NamingConventions::kindNameLookup{$varkindNameArray},$trueName,$errors);
          last;
        }
      }  
    }
  ...
  else
  {
    my $trueName = $varRef->file->lexer->lexeme($varRef->line,$varRef->column)->text;
    my $errors = checkNamingRules($trueName,$varkindNameArray,$check);
    $check->violation($var,$varRef->file,$varRef->line,$varRef->column,NAMEERROR,$CodeCheck::Libraries::NamingConventions::kindNameLookup{$varkindNameArray},$trueName,$errors) if $errors;
  }
}

#see if we need to check refs as well
sub isAdaDeepSearch{
  my ($check,$type) =@_;

  my $prefixList = scalar split(/,/,$check->option->lookup($type."prefix")) > 0;#
  my $regExp = $check->option->lookup($type."regExp");#
  my $suffixList = scalar split(/,/,$check->option->lookup($type."suffix")) > 0;#
  my $capOpt = $check->option->lookup($type."capOpt") ne "ignore";#
  my $allowConsCaps = $check->option->lookup($type."consCaps") ne "Allowed";#

  my $search = ($allowConsCaps || $capOpt || $suffixList || $regExp || $prefixList);
  return $search;
}
